...
## boolean
## default - false
### If true, updates that declare no "must-close" applications, or have one or more "must-close" applications, none of which is running, and do not require a logout or restart will be installed as part of a normal periodic background run without notifying the user. (OS X 10.10+, Munki 2.5+)
...
#### /usr/bin/defaults write /Library/Preferences/ManagedInstalls UnattendedAppleUpdates -int 3

# PerformAuthRestarts
## boolean
## default - false
### (New in Munki 3) If true, Munki will attempt to perform a filevault auth restart. See Authorized-Restarts.
#### /usr/bin/defaults write /Library/Preferences/ManagedInstalls PerformAuthRestarts -bool false
...
#### /usr/bin/defaults write /Library/Preferences/ManagedInstalls RecoveryKeyFile -string "none"
